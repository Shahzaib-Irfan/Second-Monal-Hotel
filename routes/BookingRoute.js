...
const stripe = require("stripe")(
  "sk_test_51NQplaIguDrrT8uj975KQ7XbE1Uh8887Tl8TdhhrpHrCYMt0TgGwcohRVreIo7PQCZt35AIdy2US6nBQ7LYmcw0Z007bHu5rIa"
...
router.post("/getbookingsbyuserid", async (req, res) => {
  try {
    ...
    if (booking) {
      ...
      res.json({
        success: false,
        message: "Bookings not found for the given userid",
      });
    }
  } catch (error) {
    console.error(error);
...
router.post("/cancelbooking", async (req, res) => {
  ...
  try {
    ...
    const tempObj = bookins.filter(
      (bookin) => bookin.bookingId.toString() !== id
    );
